...
Post-audit event (2026-07-23): while the routed verdicts were being adjudicated, the owner ruled that reviewer dispatch never overrides the owner's explicit word and that no committed list of models may exist ("deleted"). The fleet-global model map, its fetch-contract lint, the model-denylist lint, and the `harness-update` operator were deleted in the same change; the 2026-07-19 ruling is superseded and the 2026-07-09 evaluation archived as ephemera. Blocks below that reference the deleted apparatus record audit-time state; outcomes are marked on the affected entries (2026-07-19 block, F3, F7). The new ruling is recorded in `.agents/decisions.md`.
...
### 2026-07-19 — Model slugs get one committed home: fleet-global `.agents/model-map.json` <!-- plan-lint: allow (deleted 2026-07-23; the ruling this block audited is now superseded) -->
- EVIDENCE: Supported at audit time. `.agents/model-map.json` existed and matched the ruled seeds (`sol`→`gpt-5.6-sol`, `terra`→`gpt-5.6-terra`, v1). Shipped mechanics: `templates/playbooks/codereview.md:259-314` (map section, fetch contract, grammar), `templates/playbooks/harness-update.md:7-9` (sole write path), `templates/commands/claude/review.md:6-9` (pure alias, F6). Commits `dac2c2c`, `1dcea6a`, `d412438`, `d989054`. Two wrinkles: (a) the shipped cache schema at `codereview.md:147-152` still carried `"model": "<id>"` per tier and L245-246 still resolved a tier from the machine-local cache, contradicting L267-270 (slug text reads from the map); (b) the live gitignored `.agents/review/harnesses.local.json` still pins slugs and predates the ruling. **Post-audit: the ruling was superseded 2026-07-23 and the apparatus deleted (owner ruling); this block records the state at audit time.** <!-- plan-lint: allow (deleted files named as audit-time evidence) -->
...
- ENFORCED: Yes, at audit time — `tests/test_model_map.py::test_committed_map_passes_fetch_contract` <!-- plan-lint: allow (test file deleted 2026-07-23 with the apparatus; named as audit-time evidence) -->, `::test_committed_map_within_size_cap`, `tests/test_templates.py::test_shipped_template_text_names_no_concrete_model_ids` (denylist at `docs/harness-capabilities.md:105-118`), `::test_codereview_carries_tier_semantics`. Runtime nickname resolution was agent-prose — deliberately no standalone resolver (owner sizing, recorded in the test docstring).
- TODAY: Same design — superseded by events: the owner ruled 2026-07-23 that no committed list of models may exist, and the map, lints, and `harness-update` operator were deleted. VERDICT at audit time: HOLDS. OUTCOME: ruling superseded 2026-07-23 ("The owner's dispatch word is final"); F7 mooted.
...
- F3 (CONTRADICTED) Amend the 2026-07-09 Codex evaluation: record that `codex mcp-server` was adopted as the preferred transport 2026-07-17 (`ec7b62e`). **RESOLVED 2026-07-23 by owner ruling, wider than proposed:** the owner ruled harness-version evaluations are not durable record and the whole model-map/denylist apparatus is deleted ("deleted"); the 2026-07-09 entry was archived verbatim (no amendment) and the 2026-07-19 model-map ruling superseded. Recorded in `.agents/decisions.md` (2026-07-23, "The owner's dispatch word is final").
- F4 (HOLDS-UNSHIPPED) Paperwork rule: add one generalized completion-bookkeeping clause to `templates/AGENTS.template.md`, or amend the entry to record deliberate local scope. **LANDED 2026-07-23** (owner go): the Prime Invariants commit bullet now closes verified work's paperwork in the same motion — tracker issues closed, records synced, no fresh ask; anything beyond recording already-approved work still stops.
...
- F6 (enforcement) refresh.py self-target refusal (target = toolkit clone ⇒ refuse) + regression test. **LANDED 2026-07-23** (owner-surface Stage 2a, `9ad88a5`; guard-proven).
- F7 (template defect) Reconcile `templates/playbooks/codereview.md` cache schema (L147-152, 245-246) with the model-map section (L267-270); reconcile stale slugs in `.agents/review/harnesses.local.json`. **MOOT 2026-07-23:** the model-map section and the map itself were deleted under the owner ruling; the cache is again the sole tier-pair store (owner-named, no ritual), so the self-inconsistency no longer exists.
- F8 (enforcement) Structural dedup check for `AGENTS.template.md` (in `lint_governance` or a test pinning known duplicate pairs). **LANDED 2026-07-23** (`0b5805d`): `TemplateRuleDedup` test — phrase pins for the three historical pairs + duplicate-substantial-line detector, guard-proven.
- F9 (enforcement) Structural pin: codereview.md carries "Self-permissioning launch" + the `--allowedTools` line. **LANDED 2026-07-23** (`a83cbd3`).
- F10 (enforcement) comms-policy marker-format lint + add comms-policy.md to refresh.py's repo-owned docstring list. **LANDED 2026-07-23** (`57b8365`): marker preflight mirrors the push-policy one; docstring also records machines.md and the mechanical-repair exception.
- F11 (paperwork) Amendment pointers on the 2026-07-08, 2026-07-03 (playbooks), 2026-07-09 (banner), and 2026-06-18 entries re: 2026-07-16 strict converge and 2026-07-23 exit 5; fix the stale comment at `tests/test_templates.py:94-95`. **LANDED 2026-07-23** (`4b0a468`).
- F12 (load-time) Move the `handoff` and `plan` operator bullets' procedure text to playbooks with one-line dispatch stubs (the `drift` shape); repoint the two skills; ~300-word per-session saving. The `formerly[]` maintenance rule applies to the template change. **LANDED 2026-07-23** (owner-surface Stage 3, D4): `templates/playbooks/{catchup,handoff,plan}.md` created, bullets are now dispatch stubs, `drift` word retired fleet-wide, `toolkit` guidance verb added; template 1,977 → 1,783 words.
- F13 (minor annotations, each a separate item) repo-facts.jsonl decline line; 2026-06-27 "Step 10" citation fix; 2026-06-09 layout member list; 2026-06-10 CI-markers packet-fields sentence marked historical; duplicate-entry annotation (artifact-is-evidence merged into answer-with-words); 2026-06-10 custody-clause supersession note. **LANDED 2026-07-23** (`b11ccd7`, `ce08c02`).
