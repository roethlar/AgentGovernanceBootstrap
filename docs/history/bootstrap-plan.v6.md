...
Supersedes: `bootstrap-plan.v5.md`. v6 keeps the manifest-only design and changes the
first implementation surface from PowerShell-first to Python-first so the tool has one
cross-platform command surface for Windows, macOS, and Linux.

## What Changed From v5

v5 correctly removed generated content packets and made discovery manifest-only. v6 keeps
that design and changes implementation strategy:

- Python is the primary implementation surface.
- PowerShell and shell wrappers may exist later, but only as thin launchers around Python.
- The command examples use `python -m agent_bootstrap ...`.
- The build order starts with a Python package and tests instead of `.ps1` modules.

This is a significant plan change because it affects how the human tests the tool and how
the tool remains portable.
...
```bash
python -m agent_bootstrap discover <repo>
python -m agent_bootstrap apply <repo> --approved-review <review-packet>
python -m agent_bootstrap apply <repo> --update --approved-review <review-packet>
python -m agent_bootstrap test-current <repo>
python -m agent_bootstrap test-clean <repo> <clean-local-copy>
python -m agent_bootstrap update <repo>
```

Reasons:

- one command surface can work on Windows, macOS, and Linux
- easier for the human to test across typical development environments
- easier to unit test deterministic behavior
- avoids maintaining separate PowerShell and shell implementations before the design is
  proven

PowerShell or shell scripts may later be added as wrappers, but they must call the Python
implementation rather than duplicating behavior.

## Process Model

The bootstrap is hybrid.

The deterministic Python layer handles mechanical work:
...
Layer 1 is deterministic. `grade-agent-run.py` checks only machine-decidable signals and
...
4. Run:
   python <repo>/.agents/grade-agent-run.py <repo>/.agents/test-results/current-run.md
```
...
1. Create a minimal Python package with CLI dispatch and clean refusal for unimplemented
   modes.
...
3. Add tests for path classification, git facts, and manifest generation.
4. Run discovery against a small repo in preview mode.
5. Use the manifest to have an in-repo agent draft the first repo-specific guidance.
6. Only then implement `apply`, checks, graders, and adapters.
