...
Supersedes: `bootstrap-plan.v7.md`. v8 simplifies the workflow around a single handoff
directory inside the target repo: `.agent-bootstrap/`.

## What Changed From v7

v7 separated the core governance system from implementation-surface options, but left too
many operational choices open. v8 narrows the workflow:

- The discovery helper is external to the target repo.
- Discovery output is written inside the target repo to `.agent-bootstrap/`.
- `.agent-bootstrap/` is temporary, ignored, and never durable authority.
- `AGENTS.md` contains the handoff rule: if `.agent-bootstrap/` exists, process it first.
- A slash command may exist later, but it is not required for the basic workflow.
- Durable repo guidance remains docs/data in tracked files.
...
## Required Ignored Handoff Directory
...
`.agent-bootstrap/` exists only to pass current discovery results from the deterministic
...
If `.agent-bootstrap/` exists, treat it as temporary bootstrap input.

1. Read `.agent-bootstrap/bootstrap-review-packet.md`.
2. Read `.agent-bootstrap/repo-discovery-manifest.json`.
3. Follow the requested bootstrap or update workflow.
4. Read the suggested repo files directly from the repo.
5. Produce proposed durable guidance changes.
6. Ask for approval before writing or replacing tracked guidance.
7. After the human confirms the bootstrap output is no longer needed, delete
   `.agent-bootstrap/`.

Do not treat `.agent-bootstrap/` as durable authority.
```

This lets a user run discovery, open an agent in the repo, and rely on `AGENTS.md` to
direct the agent without retyping the workflow.

## AGENTS.md Normal Startup Rule

If `.agent-bootstrap/` does not exist, `AGENTS.md` should define normal startup behavior:

```markdown
## Session Startup

If `.agent-bootstrap/` does not exist:

1. Check git status.
2. Read `AGENTS.md` and relevant `.agents/` files.
3. Briefly report:
   - branch and dirty state
   - whether durable agent guidance is present
   - notable untracked or ignored agent-control files
   - active requested task, if any
4. Proceed with the user's request.
```

This startup status should be concise. It is not a substitute for doing the requested work.
...
Discovery produces `.agent-bootstrap/repo-discovery-manifest.json`. It contains no source
file contents.
...
Discovery also produces `.agent-bootstrap/bootstrap-review-packet.md`.
...
1. External helper writes `.agent-bootstrap/` in the target repo.
2. Target repo `AGENTS.md` handles `.agent-bootstrap/` if present.
3. Durable files are written only after approval.
