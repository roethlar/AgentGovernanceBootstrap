...
| codex | **yes on 0.146.0 — blocking via JSON deny only (verified 2026-08-01).** Repo-local `.codex/hooks.json` (Claude-schema) PreToolUse fires in exec mode behind three gates: `[features] hooks = true`, project trust (`[projects]` entry — without it the repo-local layer silently doesn't load), and hook trust (hash-pinned via interactive `/hooks`, or `--dangerously-bypass-hook-trust`). JSON `permissionDecision: "deny"` blocks (`hook: PreToolUse Blocked`); **exit 2 + stderr does NOT block** — logged `PreToolUse Failed`, tool proceeds. Historical: repo-local engine dormant on 0.143.0 (2026-07-08); global-era `pre_tool_use` fires 2026-06-29. | **not needed** — codex re-reads `AGENTS.md` after compaction natively (assumed — owner-attested 2026-07-08, no probe; consistent with codex delivering instruction files as context rebuilt per turn, outside the compacted conversation). A re-ground hook would duplicate the harness's own behavior. (The hook-engine negative above stands as the mechanism record.) | unconfirmed |
...
- 2026-07-16 — Claude Code `protect-governance.py` PreToolUse deny (repo-level `.claude/settings.json`, headless `claude -p` in a throwaway temp repo, `--allowedTools Edit`): **positive** — control edit to an unprotected file succeeded, Edit against `AGENTS.md` was blocked with the hook's message and the file was unchanged on disk. Known fail-open limitation: the interpreter chain viability-probes `py -3`, `python3`, then `python`; when none works it exits 0 silently — the invariant text and refresh's converge-to-shipped restore are the primary layers; the hook is defense in depth, Claude Code only.
- 2026-07-27 — Claude Code hook interpreter selection on Windows: **positive after correction** — the prior presence-only, `python3`-first command selected the Microsoft Store alias and exited 126 before an unprotected edit despite a working `py -3`. The shipped viability-probed command, run under Windows Git Bash against the real hook, returned 0 for an unprotected path, preserved exit 2 plus the block message for `AGENTS.md`, and returned 0 without output when only the Store aliases were visible.
- 2026-08-01 — codex 0.146.0 repo-local hooks in exec mode: **positive**, reversing the 0.143.0 negative. Differential probes (SessionStart + PreToolUse markers in `<repo>/.codex/hooks.json`, `codex exec` with stdin-piped prompt, temp dir): untrusted project fired neither hook even with `--dangerously-bypass-hook-trust` (silent non-load of the repo-local layer); adding a `[projects.'<dir>'] trust_level = "trusted"` entry fired both. Three gates total: `[features] hooks = true` (explicitly set in this machine's global config by headroom init; the default without it is unprobed), project trust, and per-handler hook trust (probe used the bypass flag; the durable path is one-time `/hooks` pinning in the TUI). Schema is Claude-compatible — same `hooks.json` shape; shell reports `tool_name: "Bash"` even when spawning pwsh on Windows; file edits report `apply_patch` (docs: `Edit`/`Write` aliases accepted as matchers).
- 2026-08-01 — codex 0.146.0 PreToolUse **deny semantics differ from Claude Code**: exit 2 + stderr reason is **negative** — logged `hook: PreToolUse Failed` and the `apply_patch` proceeded (the hooks doc at learn.chatgpt.com/docs/hooks describes exit-2 blocking, but that page tracks main). Stdout JSON `{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "deny", "permissionDecisionReason": ...}}` with exit 0 is **positive** — `hook: PreToolUse Blocked`, the file was not created, and the agent reported itself blocked and stopped. A codex port of `protect-governance.py` must emit the JSON shape, not exit 2.
- 2026-08-01 — codex hook trust pins a per-handler sha256 over the handler config, not just the command string (the two pinned global hooks share one command yet carry different hashes under different events; exact hash input unconfirmed). Consequence: any refresh-shipped change to hook config invalidates the pin, so each governed repo needs a one-time interactive re-trust (`/hooks`) after hook-config changes. Docs surface: developers.openai.com/codex/hooks now redirects to learn.chatgpt.com/docs/hooks; event list there includes `Stop`, `SessionEnd`, `UserPromptSubmit`, `PostToolUse`, `PreCompact`/`PostCompact` — none of those probed on codex yet (`Stop`-blocking still unconfirmed).
- 2026-08-01 — Claude Code PreToolUse **JSON deny shape: positive** (headless `claude -p` in a throwaway temp repo, `--allowedTools Edit`, repo-level `.claude/settings.json` hook emitting `{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "deny", ...}}` with exit 0): the protected edit was blocked with the hook's reason surfaced to the model, the control edit succeeded. Claude Code therefore honors both exit-2 and the JSON deny; codex honors only the JSON deny — the shipped `protect-governance.py` uses the JSON shape as its single code path (2026-08-01 plan, slice 1).
- 2026-08-01 (guard-port slice 3 probes, codex 0.146.0, Windows): hook `command` strings are **not** run through a POSIX shell on Windows — an sh `if/then` viability chain silently never executes, while plain argv (`py -3 <script>`) does; the shipped `.codex/hooks.json` therefore carries `commandWindows` in the plain form and keeps the sh chain in `command` for POSIX hosts (assumed there, not yet probed on macOS/Linux). A handler object with `commandWindows` but no `command` **invalidates the entire hooks.json** — no hooks fire, no warning. Matcher `apply_patch|Edit|Write` fires for `apply_patch`. The hook process does not run from the repo root: the payload's `cwd` field is the root, and `protect-governance.py` now resolves targets against it (env `CLAUDE_PROJECT_DIR`, then payload `cwd`, then process cwd; discriminating unit test uses an absolute patch target from a foreign cwd). Live proof, exact shipped config + canonical script: codex `apply_patch` against `AGENTS.md` denied (`hook: PreToolUse Blocked`), file unchanged, twice. One anomaly for the record: a pre-fix live run on the slice-2 script failed to block and did not reproduce locally; superseded by the fixed script's live blocks.
- 2026-08-01 (guard-port slices 4–5, codex 0.146.0): the codex-native non-interactive trust surface is the experimental **app-server** (newline-delimited JSON-RPC over stdio; no `codex hooks` subcommand exists). Verified flow: `initialize` → `hooks/list` (params `{"cwds": [<repo>]}`; each hook returns camelCase `key`, `currentHash`, `trustStatus`, `sourcePath`, `command`) → `config/batchWrite` upserting `hooks.state.<key> = {"trusted_hash": <currentHash>}` (`filePath: null` targets the user config; codex validates and writes its own TOML) → re-list confirms `trusted`. Positive live: the fixture repo's hook flipped untrusted→trusted, and a subsequent `codex exec` **without** `--dangerously-bypass-hook-trust` blocked an `apply_patch` against `AGENTS.md` (`hook: PreToolUse Blocked`, file unchanged) — the full production chain: project trust + app-server-pinned hook + shipped `.codex/hooks.json` + canonical script. `refresh.py` ships this as `codex_trust_pin`/`offer_codex_hook_trust` (offered only in a run that installs/updates/restores the hook config, TTY-gated, owner-confirmed, fail-open). Wire shapes for the installed version regenerable via `codex app-server generate-json-schema`.
