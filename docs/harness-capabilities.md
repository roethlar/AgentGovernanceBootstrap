...
| agy | yes — Claude-style events in **global** `~/.gemini/settings.json` (docs: antigravity.google/docs/hooks) | **POSITIVE — repo-level `.agents/hooks.json` SessionStart fires in a trusted workspace** (owner live check 2026-07-08, agy 1.1.0: marker printed at the **first prompt**, not at UI startup — which is when the reminder needs to be in context anyway). Re-ground re-shipped (`templates/hooks/agy/hooks.json`, unmatchered SessionStart — fires once per session). Untrusted workspaces: silent (trust gate). | no — `Stop` passive (`{"decision":"allow"}` only) |
