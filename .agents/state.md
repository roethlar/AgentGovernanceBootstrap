...
- 2026-06-25: owner approved implementing a **stall-not-length** governance invariant (iterative processes escalate on a cycle that banks no verifiable progress, never on duration; must not break long converging runs). Design is settled and recorded as the 2026-06-25 entry in `.agents/decisions.md`; next step is a `plan` for the exact wording + `templateVersion` bump before it edits durable guidance. See decisions.md for the full design — not duplicated here.
