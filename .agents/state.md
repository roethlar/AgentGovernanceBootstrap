...
- 2026-06-25: all work above pushed to GitHub (`origin/master` at `76dfac6`); the gitea mirror follows.
...
- 2026-06-25: the **AGENTS.md portability + write-authority boundary** spec (`docs/superpowers/specs/2026-06-25-agents-portability-boundary-design.md`) is written and codex-reviewed but **awaits owner review, then a `plan`**. It defines two invariants (AGENTS.md is governance-only and portable; written only by a gated bootstrap/update run) enforced in three layers (invariant everywhere; a non-blocking pre-edit reminder hook on Claude Code + Codex only; `drift` as cross-harness backstop). Layer-2 hooks and retroactive cleanup of existing bad `AGENTS.md` are each split to their own follow-on specs (not yet written).
