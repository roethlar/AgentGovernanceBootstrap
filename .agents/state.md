...
- 2026-06-25: owner wants to implement a **stall-not-length** governance hook. Iterative agent processes (loops, sweeps, long autonomous runs) should escalate to a human on *stall* — a cycle that banks no verifiable progress — never on duration. Length is not a failure mode: a 30-hour run that closes a verified delta each cycle is healthy; a short loop that re-fixes the same thing is not. Same family as the repo's drift and vacuous-test rules — "cycle with no observable delta" at the loop level. Serves the token-efficiency mission by killing non-converging runs early without capping productive long ones (must not break Fable/ultracode-style long autonomous runs). Next step: a `decision` + a `plan` for the exact invariant wording and where it lands (likely a Universal Invariant in `AGENTS.md`/template, mirrored to `.agents/`), before any edit to durable guidance.
