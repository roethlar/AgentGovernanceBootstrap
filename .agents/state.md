...
- 2026-06-25: all work above (through the synchronous-review/stall-not-length batch) pushed to GitHub (`origin/master` at `76dfac6`); the gitea mirror follows. The governance-boundary work that followed is committed locally and **not yet pushed** (push offer pending).
...
- 2026-06-25: the **AGENTS.md governance boundary** (all three layers) is implemented and Adopted (see "Now" above and the 2026-06-25 boundary decision). Two follow-ons remain, each its own future spec, NOT done: (a) retroactive cleanup of already-bootstrapped repos whose `AGENTS.md` already carries repo-specifics (intersects the 2026-06-22 update-route reconciliation machinery); (b) whether the mechanizable path/name leak scan folds into the queued `governance-lint` playbook (Open Decision, 2026-06-22) or stays an agent-judgment `drift` step.
