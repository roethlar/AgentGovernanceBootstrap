...
- 2026-06-20 (continuation): three of the four remaining Open Decisions were adopted as scoped commits. (1) `templates/decisions.template.md` now models the Open-queue / Adopted lifecycle. (2) A `playbook <name>` operator was added to `templates/AGENTS.template.md` Operator Requests with a `templates/commands/claude/playbook.md` wrapper, wired into every wrapper-audit enumeration. (3) The owner's external two-agent review-loop runbook was revised (owner-gated merge, status nested under `.agents/review/`, verification deferred to the repo's command, harness-agnostic capabilities) and shipped as `templates/playbooks/reviewloop.md`, establishing the new `templates/playbooks/` set. All three are product-only; this repo's own self-application stays deferred to the bootstrap re-run. The fourth item (forceful enforcement of load-bearing invariants) was NOT decided: an agent implemented a "Prime Invariants" mechanism without owner approval, and it was reverted. Its mechanism remains the owner's to choose; the finding stays Open. See `.agents/decisions.md` for the authoritative entries.

## Next

- One Open Decision remains: "Load-bearing invariants need forceful enforcement" (2026-06-20). Its direction is recorded but the specific mechanism is the owner's to decide; it was wrongly implemented-then-reverted this session, so it stays Open pending an owner decision. The `.agents/decisions.md` "Open Decisions" queue stays the authoritative list; it is not duplicated here.
- Once that item is decided and the queue is clear, re-run the bootstrap on this repo once to bring its own self-application current: add the Operator Requests section and `.claude/commands/` wrappers (including the new `playbook` wrapper), apply the adopted product invariants to this repo's own `AGENTS.md`, and archive the adopted decisions into `docs/history/` per the archiving rule. This repo intentionally lags the product it ships until that re-run.
