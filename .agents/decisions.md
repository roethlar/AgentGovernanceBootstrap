...
### 2026-09-10 — Restore behavior; wording-only reductions

Status: Active

Owner: "revert EVERYTHING. ALL FUNCTIONALITY restored"; then reduce tokens
without functionality changes. Restore f1497c4 before the disputed pass,
retaining the owner's later readable-response/no-format-retry rule and
Headroom removal. This supersedes the waste-pass approval claim in 9aad4ae.

Tools, hooks, skill triggers, catchup/drift, procedures and verification
policies return to baseline. All 226 test cases return; only review-format
assertions change to preserve the same substantive outcomes in prose.
AGENTS wording preserves its baseline rules: 770 → 707 o200k_base and
768 → 707 cl100k_base tokens, below the previous 711/710 ceilings.
Template/procedure Markdown: 18,641 → 18,268 o200k_base tokens. These are
text counts, not runtime savings or proof of behavior equivalence.

Verification: Python 3.14 full suite, 226 tests passed; `git diff --check`
